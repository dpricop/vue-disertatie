    ...
    <div class="row">
      <div class="col-auto col-md-6 col-lg-4">
        <table class="table table-bordered bg-white mb-0">
          ...
          <tbody>
            <tr v-for="(item, index) of dataSource" :key="item.IdLeadStatus">
              <td>{{ index + 1 }}</td>
              ...
              <td>
                <a
                  ...
                  data-bs-target="#CrmLeadStatusForm"
                >
...
<script>
import CrmLeadStatusForm from "./CrmLeadStatusForm.vue";
...
export default {
  ...
  methods: {
    ...
    addItem() {
      ...
      this.axios
        .post("http://localhost:8080/crmLeadStatus/Create", data)
        .then((response) => {
    ...
    editItem(item) {
      this.editItemData.IdLeadStatus = item.IdLeadStatus;
      this.editItemData.StatusNume = item.StatusNume;
    },
    editedItem(params) {
      const data = new FormData();
      data.set("id", params.IdLeadStatus);
      data.set("values", JSON.stringify(params));
      this.axios
        .put("http://localhost:8080/crmLeadStatus/Edit", data)
        .then((response) => {
          if (response.data.status == 1 && response.data.count == 1) {
            this.reload();
            var formModal = window.bootstrap.Modal.getInstance(
              document.getElementById("CrmLeadStatusForm")
            );
            formModal.hide();
          }
        })
    ...
    deleteItem(item, index) {
      this.axios
        .delete(
          "http://localhost:8080/crmLeadStatus/Delete/" + item.IdLeadStatus
        )
        .then((response) => {
          if (response.data.status == 1 && response.data.count == 1)
